...
import 'package:webtesthttp/provider/news_item_privider.dart';
                  ...
                  if (snapshot.connectionState == ConnectionState.done) {
                    if (snapshot.hasError) {
                      ...
                      return ListView.separated(
                        ...
                        itemCount: min(itemsPerPage, newsDate!.length - currentPage * itemsPerPage),
                        itemBuilder: (BuildContext context, int index) {
                          int itemIndex = (newsDate!.length - 1) - (currentPage * itemsPerPage + index); // 내림차순으로 항목의 실제 인덱스 계산
                          return Container(
...
class NoticeState extends State<Notice> {
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            Expanded(
              child: buildNewsList(deviceWidth),
            ),
            buildPagenation(startPage, endPage),
          ],
        ),
      ),
      drawer: const BaseDrawer(),
  ...
  FutureBuilder<List<BoardP>?> buildNewsList(double deviceWidth) {
      ...
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (snapshot.connectionState == ConnectionState.done) {
          if (snapshot.hasError) {
            ...
            List<BoardP>? data = snapshot.data;
            print('FutureBuilder FutureBuilder');
            return ListView.separated(
              primary: false,
              scrollDirection: Axis.vertical,
              shrinkWrap: true,
              // itemCount: snapshot.data!.length,
              itemCount: min(itemsPerPage, data!.length - currentPage * itemsPerPage),
              itemBuilder: (BuildContext context, int index) {
                int itemIndex = (data!.length - 1) - (currentPage * itemsPerPage + index); // 내림차순으로 항목의 실제 인덱스 계산
                return Column(
                  children: [
                    ChangeNotifierProvider(
                      create: (_) => NewsProvider(data[itemIndex]),
                      child: NewsItem(itemIndex: itemIndex, deviceWidth: deviceWidth, widget: widget),
                    ),
                    //Text("like:${data[itemIndex].likecount}")
                  ],
                );
              },
              separatorBuilder: (BuildContext context, int index) => const Divider(),
  ...
  //하단 페이지 번호
  Row buildPagenation(int startPage, int endPage) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        IconButton(
